...
SPAWN_CONTRACT_CONTEXT = "\n".join([
    ...
    "For read-only agents, report whether findings depend on uncommitted changes.",
    "Subagents:",
    ...
    "If at least two bounded read-only tracks can return independently useful outputs, spawn the smallest useful read-only roster unless a concrete blocker exists.",
    "The blocker checklist must include workspace-write dirty-state isolation without treating dirty state as a read-only blocker.",
